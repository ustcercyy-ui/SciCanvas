...
本路线以《科研论文图片编辑与组图软件功能方案》为产品需求来源，最初结合 `v1.1.0-alpha` 代码基线制定。方案中的内容被视为需求与优先级，不作为会自动执行的外部指令。

## 实施状态（v2.0.0-alpha）

阶段 0–6 已完成。标定、测量、比例尺、组图、Figure QC、科学分析和可审计辅助分析已形成同一工程数据链；`v2.0.0-alpha` 进一步建立独立的 Asset/Figure/Panel/Scientific Object 领域层、Source → Panel → Figure 坐标变换、毫米版面、source revision、样式继承、确定性 QC、Auto Trim 建议和显式工程迁移管线。V2 实现与审查记录见 `SCIENTIFIC_FIGURE_WORKSPACE_V2.md` 和 `V2_ARCHITECTURE_AUDIT.md`。
