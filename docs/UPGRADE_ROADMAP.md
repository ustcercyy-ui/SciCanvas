...
阶段 0–6 已完成。标定、测量、比例尺、组图、Figure QC、科学分析和可审计辅助分析已形成同一工程数据链；`v1.2.1-alpha` 补齐画布缩放/平移、裁剪手柄、统一图层选择与测量对象编辑，`v1.2.2-alpha` 修复空工程裁剪层、裁剪手势和拖动性能，发布验收记录见 `RELEASE_1.2_QA.md`。
