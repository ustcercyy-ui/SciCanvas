...
internal enum PlotTextAnchor { Start, Middle, End }

internal readonly record struct PlotPoint(double X, double Y);

internal readonly record struct PlotRect(double X, double Y, double Width, double Height)
{
...
internal abstract record PlotPrimitive;
internal sealed record PlotLine(PlotPoint A, PlotPoint B, string Stroke, double Width, PlotLineStyle Dash = PlotLineStyle.Solid) : PlotPrimitive;
internal sealed record PlotRectangle(PlotRect Bounds, string? Fill, string? Stroke = null, double Width = 0, PlotLineStyle Dash = PlotLineStyle.Solid) : PlotPrimitive;
internal sealed record PlotEllipse(PlotRect Bounds, string? Fill, string? Stroke = null, double Width = 0) : PlotPrimitive;
internal sealed record PlotPolygon(IReadOnlyList<PlotPoint> Points, string? Fill, string? Stroke = null, double Width = 0) : PlotPrimitive;
internal sealed record PlotText(string Value, double X, double Y, TextStyle Style, double FontPixels, PlotTextAnchor Anchor = PlotTextAnchor.Start) : PlotPrimitive;
internal sealed record FigurePlotScene(IReadOnlyList<PlotPrimitive> Primitives, PlotRect Chart);

internal static class FigurePlotSceneBuilder
{
    public static FigurePlotScene Build(FigurePlotPanelExportItem panel, FigureGlobalStyle figureStyle, int dpi)
    {
        panel.EnsureValid();
        ArgumentOutOfRangeException.ThrowIfNegativeOrZero(dpi);
        PlotObject plot = panel.Plot;
        PlotTypography typography = panel.ResolveTypography(figureStyle).Value;
        var destination = new PlotRect(
        ...
        Series series = BuildSeries(panel.Projection, plot.PlotType);
        ...
        output.Add(new PlotText(
            ...
            annotationFont,
            PlotTextAnchor.End));
        return new FigurePlotScene(output, chart);
    }
    ...
    private static void AddHeatmap(List<PlotPrimitive> output, PlotRect chart, PlotObject plot, Bounds bounds, IReadOnlyList<Sample> samples)
    {
        ...
        foreach (Sample sample in samples.Where(sample => sample.Value.HasValue))
        {
            ...
            {
                output.Add(new PlotRectangle(cell, $"#FF{red:X2}{green:X2}{blue:X2}"));
            }
        }
    }
...
internal static class WpfPlotPanelRenderer
{
    ...
    {
        foreach (PlotPrimitive primitive in FigurePlotSceneBuilder.Build(panel, style, dpi).Primitives)
        {
            switch (primitive)
            {
                case PlotLine line:
